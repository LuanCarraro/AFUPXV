...
export const Course = () => {
    ...
    const { id } = useParams();
    const navigate = useNavigate(); // Para navegar para o quiz
    ...
    // Função para alternar a expansão dos módulos
    ...
    // Redireciona para o quiz do módulo e aula selecionados
    const handleFazerAtividade = (moduloIndex, aulaIndex) => {
        navigate(`/mysubjects/course/${id}/quiz/${moduloIndex}/${aulaIndex}`);
    };
                        ...
                        {curso.modulos.map((modulo, index) => (
                            <div key={index} style={{ marginBottom: '20px' }}>
                                <div
                                    ...
                                    style={{ cursor: 'pointer', fontSize: '18px', fontWeight: 'bold', padding: '10px', backgroundColor: '#333', color: 'white', borderRadius: '5px' }}
                                >
                                    {modulo.nome}
                                        ...
                                        {modulo.aulas.map((aula, aulaIndex) => (
                                            <div key={aulaIndex} style={{ marginBottom: '15px' }}>
                                                <div className="lesson-row" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                                    <div style={{ flex: 1 }}>
                                                        <strong>{aula.nome_aula}</strong>
                                                    </div>
                                                    <button className="btn btn-primary" style={{ marginLeft: '10px' }} onClick={() => window.open(aula.link_video, '_blank')}>
                                                        Ver Vídeo
                                                    </button>
                                                </div>
                                                <div className="activities-row" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '10px' }}>
                                                    <div style={{ flex: 1 }}>
                                                        <span>Atividades sobre {modulo.nome}</span>
                                                    </div>
                                                    <button
                                                        className="btn btn-secondary"
                                                        onClick={() => handleFazerAtividade(index, aulaIndex)} // Passando moduloIndex e aulaIndex
                                                    >
                                                        Fazer Atividades
                                                    </button>
                                                </div>
